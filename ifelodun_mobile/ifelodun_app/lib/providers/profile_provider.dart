...
class ProfileProvider extends ChangeNotifier {
  ...
  Future<void> fetchProfile() async {
    ...
    try {
      ...
      debugPrint('[ProfileProvider] GET '+url.toString()+' -> '+res.statusCode.toString());
      debugPrint('[ProfileProvider] Response body: '+res.body);

      if (res.statusCode == 200) {
        final decoded = jsonDecode(res.body);
        debugPrint('[ProfileProvider] Decoded type: '+decoded.runtimeType.toString());
        if (decoded is Map<String, dynamic>) {
          debugPrint('[ProfileProvider] Top-level keys: '+decoded.keys.join(','));
        }
        // Accept both { ...profile } and { data: { ...profile } }
        final payload = decoded is Map<String, dynamic>
            ? (decoded['data'] is Map<String, dynamic>
                ? decoded['data']
                : decoded)
            : decoded;
        profile = MemberProfile.fromJson(payload);
      } else {
