...
class ValidationMiddleware {
    ...
    try {
      ...
      for (const contribution of contributions) {
        ...
        if (memberResult[0].membership_status !== "active") {
          return ResponseHandler.error(
            res,
            `Member ID ${memberId} is not active`,
            400
          );
        }
        ...
        if (amount < settings.minimum_monthly_contribution) {
          return ResponseHandler.error(
            res,
            `Contribution for member ID ${memberId} must be at least ₦${settings.minimum_monthly_contribution}`,
            400
          );
        }

        // Optional: Re-enable duplicate check if needed
        ...
        //   return ResponseHandler.error(
        //     res,
        //     `Member ID ${memberId} has already contributed for this period`,
        //     400
        //   );
        // }
      }
  ...
  // Keep logValidation if you plan to use it
